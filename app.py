...
#         if numeric_cols:
#             if st.button(f"Histogram of {numeric_cols[0]}", key="viz_1"):
#                 st.session_state["suggested_query"] = f"Create a histogram of {numeric_cols[0]}"
#                 st.rerun()
#         if st.button("Correlation heatmap", key="viz_2"):
#             st.session_state["suggested_query"] = "Create a correlation heatmap"
#             st.rerun()
#         if numeric_cols and categorical_cols:
#             if st.button(f"Box plot by {categorical_cols[0]}", key="viz_3"):
#                 st.session_state["suggested_query"] = f"Create box plots of numeric columns by {categorical_cols[0]}"
#                 st.rerun()
    
#     with col2:
#         st.markdown("**🔍 Analysis**")
#         if st.button("Summarize dataset", key="analysis_1"):
#             st.session_state["suggested_query"] = "Summarize the dataset"
#             st.rerun()
#         if st.button("Show missing values", key="analysis_2"):
#             st.session_state["suggested_query"] = "Show missing values breakdown"
    ...
    # # Get numeric and categorical columns for better suggestions
    # numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    # categorical_cols = df.select_dtypes(include=['object', 'category']).columns.tolist()
    ...
    #     st.markdown("**📊 Visualizations**")
    #     viz_suggestions = [
    #         f"Show me a histogram of {numeric_cols[0]}" if numeric_cols else "Show me a histogram",
    #         f"Create a bar chart by {categorical_cols[0]}" if categorical_cols else "Create a bar chart",
    #         "Plot a correlation heatmap",
    #         "Show box plots for all numeric columns"
    #     ]
    #     for i, suggestion in enumerate(viz_suggestions):
    #         if st.button(suggestion, key=f"viz_{i}", use_container_width=True):
    ...
    #         "Show me missing values breakdown",
    #         "Identify outliers in the data"
    #     ]
    #     for i, suggestion in enumerate(analysis_suggestions):
    #         if st.button(suggestion, key=f"analysis_{i}", use_container_width=True):
    #             st.session_state["suggested_query"] = suggestion
    #             st.rerun()
    ...
    #         f"Show top 10 by {numeric_cols[0]}" if numeric_cols else "Show top 10 records",
    #         f"Compare {categorical_cols[0]} groups" if categorical_cols else "Compare categories",
    #         "Find interesting patterns"
    #     ]
    #     for i, suggestion in enumerate(insight_suggestions):
    #         if st.button(suggestion, key=f"insight_{i}", use_container_width=True):
    #             st.session_state["suggested_query"] = suggestion
    #             st.rerun()